    ...
    if (report.status === 'pending') {
        return (
            <div className="flex items-center gap-2">
                <button
                    onClick={() => onInvestigate(report.id)} // 🔥 gọi cha
                ...
                <button
                    onClick={() => onReject(report.id)} // 🔥 gọi cha
                    className="px-3 py-1.5 bg-yellow-600 text-white rounded-lg hover:bg-yellow-700 text-sm font-medium flex items-center gap-1"
                >
                    <X className="w-4 h-4" />
                    Từ chối
                </button>
